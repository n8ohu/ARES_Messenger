...
namespace ARES_Messenger
{
    public partial class Radio : Form
    {
            ...
            else if (strModel == "Icom 7200" | strModel == "Icom 7600")
            {
                ...
                if (strReply.Length == 9 && (strReply(2) == "F1" & strReply(4) == "1A"))
                {
                    if (strModel == "Icom 7200")
                    {
                        ...
                        {
                            case "0":
                                enmMuxState = MuxState.Mic;
                                queSessionEvents.Enqueue("  *** Initial IC-7200 audio mux value = Mic" + Constants.vbCrLf);
                                break;
                            case "1":
                                enmMuxState = MuxState.Aux1;
                                queSessionEvents.Enqueue("  *** Initial IC-7200 audio mux value = Aux" + Constants.vbCrLf);
                                break;
                            case "2":
                                enmMuxState = MuxState.MicPlusAux1;
                                queSessionEvents.Enqueue("  *** Initial IC-7200 audio mux value = Mic+Aux" + Constants.vbCrLf);
                                break;
                            case "3":
                                enmMuxState = MuxState.InternalUSB;
                                queSessionEvents.Enqueue("  *** Initial IC-7200 audio mux value = USB" + Constants.vbCrLf);
                                break;
                            default:
                                enmMuxState = MuxState.Unknown;
                                break;
                        }
                    }
                }
                else if (strReply.Length == 10 && (strReply(2) == "F1" & strReply(4) == "1A"))
                {
                    if (strModel == "Icom 7600" | strModel == "Icom 7100")
                    {
                        switch (strReply(8))
                        {
                            case "0":
                                enmMuxState = MuxState.Mic;
                                queSessionEvents.Enqueue("  *** Initial " + strModel + " audio mux value = Mic" + Constants.vbCrLf);
                                break;
                            case "1":
                                enmMuxState = MuxState.Aux1;
                                queSessionEvents.Enqueue("  *** Initial " + strModel + " audio mux value = Aux" + Constants.vbCrLf);
                                break;
                            case "2":
                                enmMuxState = MuxState.MicPlusAux1;
                                queSessionEvents.Enqueue("  *** Initial " + strModel + " audio mux value = Mic+Aux" + Constants.vbCrLf);
                                break;
                            case "3":
                                enmMuxState = MuxState.InternalUSB;
                                queSessionEvents.Enqueue("  *** Initial " + strModel + " audio mux value = USB" + Constants.vbCrLf);
                                break;
                            default:
                                enmMuxState = MuxState.Unknown;
                                break;
        ...
        private void RestoreMuxState()
        {
            if (strModel == "Kenwood TS-590S")
            {
                if (enmMuxState == MuxState.Aux2)
                {
                    SendASCIICommand("EX06300000;");
                    queSessionEvents.Enqueue("  *** Restore TS-590S audio mux to ACC2" + Constants.vbCrLf);
                    Thread.Sleep(1000);
                }
                else if (enmMuxState == MuxState.InternalUSB)
                {
                    SendASCIICommand("EX06300001;");
                    queSessionEvents.Enqueue("  *** Restore TS-590S audio mux to USB" + Constants.vbCrLf);
                    Thread.Sleep(1000);
                }
            ...
            else if (strModel == "Icom 7200")
            {
                ...
                {
                    case MuxState.Mic:
                        queSessionEvents.Enqueue("  *** Restore Icom 7200 audio mux to Mic" + Constants.vbCrLf);
                        bytCommand(7) = 0;
                        break;
                    case MuxState.Aux1:
                        queSessionEvents.Enqueue("  *** Restore Icom 7200 audio mux to Aux" + Constants.vbCrLf);
                        bytCommand(7) = 1;
                        break;
                    case MuxState.MicPlusAux1:
                        queSessionEvents.Enqueue("  *** Restore Icom 7200 audio mux to Mic+Aux" + Constants.vbCrLf);
                        bytCommand(7) = 2;
                        break;
                    case MuxState.InternalUSB:
                        queSessionEvents.Enqueue("  *** Restore Icom 7200 audio mux to USB" + Constants.vbCrLf);
                        bytCommand(7) = 3;
                        break;
                    default:
                        return;
            ...
            else if (strModel == "Icom 7600")
            {
                ...
                {
                    case MuxState.Mic:
                        queSessionEvents.Enqueue("  *** Restore Icom 7600 audio mux to Mic" + Constants.vbCrLf);
                        bytCommand(8) = 0;
                        break;
                    case MuxState.Aux1:
                        queSessionEvents.Enqueue("  *** Restore Icom 7600 audio mux to Aux" + Constants.vbCrLf);
                        bytCommand(8) = 1;
                        break;
                    case MuxState.MicPlusAux1:
                        queSessionEvents.Enqueue("  *** Restore Icom 7600 audio mux to Mic+Aux" + Constants.vbCrLf);
                        bytCommand(8) = 2;
                        break;
                    case MuxState.InternalUSB:
                        queSessionEvents.Enqueue("  *** Restore Icom 7600 audio mux to USB" + Constants.vbCrLf);
                        bytCommand(8) = 3;
                        break;
                    default:
                        return;
        ...
        private void ProcessRadioResponse(string strResponse)
        {
            ...
            try
            {
                ...
                {
                    Log("Yaesu 8x7 Radio Response: " + strResponse);
            ...
            {
                Exceptions("[RadioResponse] " + strResponse + Constants.vbCr + " Err:" + ex.ToString);
            }
        }
        ...
        private void OnRadioResponse(object s, SerialDataReceivedEventArgs e)
        {
            ...
            else if (strModel.StartsWith("Kenwood") | strModel.StartsWith("Flex") | strModel.StartsWith("Elecraft") | strModel == "Yaesu FT-450" | strModel == "Yaesu FT-2000" | strModel == "Yaesu FT-950")
            {
                if (objControlPort != null & objControlPort.IsOpen)
                {
                    ...
                    for (int i = 1; i <= intBytesToRead; i++)
                    {
                        strRead = Strings.Chr(objControlPort.ReadByte);
                        if (strRead == ";")
                        {
                            if (RadioResponse != null)
                            {
